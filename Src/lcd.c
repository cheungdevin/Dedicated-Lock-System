...
{	
	LCD_Write_Cmd ( CMD_Set_COLUMN ); 	  //括号内等同0x2a			
	LCD_Write_Data ( usCOLUMN >> 8  );	  //横向起始点位地址的前8位
	LCD_Write_Data ( usCOLUMN & 0xff  );	 //横向起始点位地址的后8位
	LCD_Write_Data ( ( usCOLUMN + usWidth - 1 ) >> 8  );   //横向结束点位地址的前8位
	LCD_Write_Data ( ( usCOLUMN + usWidth - 1 ) & 0xff  ); //横向结束点位地址的后8位

	LCD_Write_Cmd ( CMD_Set_PAGE ); 			//括号内等同0x2b    
	LCD_Write_Data ( usPAGE >> 8  );			//纵向起始....前8位
	LCD_Write_Data ( usPAGE & 0xff  );		//纵向起始....后8位
	LCD_Write_Data ( ( usPAGE + usHeight - 1 ) >> 8 );     //纵向结束....前8位
	LCD_Write_Data ( ( usPAGE + usHeight - 1) & 0xff );		 //纵向结束....后8位
...
{	
	...
	uint16_t a = LR;
	uint16_t b = SR;
	LCD_DrawDot(usC-a,usP,usColor);
	LCD_DrawDot(usC+a,usP,usColor);
	LCD_DrawDot(usC,usP-b,usColor);
	LCD_DrawDot(usC,usP+b,usColor);
	
	for(double xcor = 0; xcor < a  ; xcor++){ 
			double ycor = sqrt((1-(xcor*xcor/(a*a))) * b*b);
			
				LCD_DrawDot(usC+xcor , usP+ycor ,usColor);
		  	LCD_DrawDot(usC-xcor , usP+ycor ,usColor);
				LCD_DrawDot(usC+xcor , usP-ycor ,usColor);
				LCD_DrawDot(usC-xcor , usP-ycor ,usColor);
			};

	for(double ycor = 0; ycor < b  ; ycor++){ 
			double xcor = sqrt((1-(ycor*ycor/(b*b))) * a*a);
